 ...
 *
 * Calculates performance scores for volleyball actions based on
...
import { getVolleyballBenchmarks } from './volleyballBenchmarks';
import {
    scoreWithPreference,
...
/**
 * Calculates the complete volleyball performance score
 *
 * @param metrics - Calculated metrics from the pose analysis
 * @param action  - Specific volleyball action (spike | serve | block | set)
...
export function calculateVolleyballScore(
    ...
    const benchmarks = getVolleyballBenchmarks(action);

    if (!benchmarks) {
        console.warn(`[VolleyballScoring] No benchmarks for action: ${action}`);
    ...
    if (metricsIncluded < benchmarks.minRequiredMetrics) {
        console.warn(
            `[VolleyballScoring] Insufficient metrics: ${metricsIncluded}/${benchmarks.minRequiredMetrics} required`
        );
        return {
            overallScore: 0,
            breakdown,
    ...
    console.log(
        `[VolleyballScoring] Calculated score in ${(endTime - startTime).toFixed(2)}ms:`,
...
function createEmptyResult(reason: string): ScoringResult {
    console.error(`[VolleyballScoring] ${reason}`);
    return {
        overallScore: 0,
        breakdown: {},
        details: {},
